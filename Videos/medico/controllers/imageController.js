...
exports.uploadCarouselImage = async (req, res) => {
  try {
    ...
    // ✅ Prevent duplicate names for same type
    ...
    const image = new Image({
      name,
      url,
      description,   // ✅ Save description if provided
      type
    });
    ...
    console.error('❌ uploadCarouselImage error:', err);
...
exports.updateCarouselImage = async (req, res) => {
  try {
    const { name, type, description } = req.body; // ✅ also get description
    const id = req.params.id;

    // ✅ Prepare update data
    const updateData = { name, description };

    // ✅ Prevent duplicate name for same type (excluding current image)
    const existing = await Image.findOne({
      _id: { $ne: id },
    ...
    console.error('❌ updateCarouselImage error:', err);
    res.status(500).json({ message: err.message });
  }
};
